  ...
  return (
    <div className="fixed left-0 top-0 h-screen w-64 bg-slate-900 text-slate-300 flex flex-col z-50 shadow-2xl">
      <div className="p-6 flex items-center gap-3 border-b border-slate-800">
        <span className="text-xl font-bold text-white italic">Smart<span className="text-indigo-400">MRP</span></span>
      </div>
      <nav className="flex-1 p-4 space-y-1 mt-2">
        {menu.map((item) => (
          <div key={item.label} onClick={() => navigate(item.path)}
            className={`flex items-center gap-4 p-3.5 rounded-xl cursor-pointer transition-all ${
              location.pathname === item.path ? "bg-indigo-600 text-white shadow-lg font-bold" : "hover:bg-slate-800"
            }`}>
      ...
      <div className="p-4 border-t border-slate-800">
        <button onClick={handleLogout} className="w-full flex items-center gap-3 p-3.5 rounded-xl text-red-400 hover:bg-red-500/10 font-bold transition-all">
